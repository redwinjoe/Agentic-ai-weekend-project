"""The whole project in one command, for the classroom.
    ...
    python -m scripts.demo --crash    # kill the run right after raising complaint, resume, count
...
QUESTIONS = [
    ("22CS045", "My tap is leaking in Room 101, please raise a complaint, assign warden, and notify me."),
    ("22EC031", "Can I raise another Plumbing complaint for Room 103?"),
]
...
def counts(db) -> str:
    return (f"complaints {db.count('complaint')}   notifications {db.count('notification')}"
...
def main() -> None:
    ...
    tmp = tempfile.mkdtemp(prefix="hostel-demo-")
    os.environ["AGENT_DB"] = os.path.join(tmp, "agent.db")
    os.environ["HOSTEL_DB"] = os.path.join(tmp, "hostel.db")
    os.environ["LIBRARY_DB"] = os.environ["HOSTEL_DB"]
        ...
        if a.crash:
            ...
            def once_then_die(key, tool_name, effect):
                ...
                if tool_name == "raise_complaint":
                    raise Crash()        # complaint and its key are committed; nothing after is
                return result
            ...
            try:
                ...
                db.once = real_once
                print(f"\n  {RED}worker-A died right after writing the complaint{RESET}")
    ...
    if a.crash:
        ok = db.count("complaint") == 2 and db.count("notification") == 1      # seed has 1 complaint
        print(f"{GREEN}PASS: one new complaint, one text{RESET}" if ok else f"{RED}FAIL: duplicates{RESET}")
